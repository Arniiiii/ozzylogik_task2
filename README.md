[![Actions Status](https://github.com/Arniiiii/ozzylogik_task2/workflows/MacOS/badge.svg)](https://github.com/Arniiiii/ozzylogik_task2/actions)
[![Actions Status](https://github.com/Arniiiii/ozzylogik_task2/workflows/Windows/badge.svg)](https://github.com/Arniiiii/ozzylogik_task2/actions)
...
the connection.
Multiple client connections are not required, but will be a plus.
To make a client program that connects to the server, receives the file, saves it
to the hard disk, and then exits.

    Translated from Ukrainian via deepl.com

The server can be got using next commands:
```
cmake -S ./server -B ./build
cmake --build ./build --target Task2Server --parallel $(nproc)
./build/server/Task2Server
```
    
The client can be either netcat:
```
nc  127.0.0.1 8001 > ./result.txt < /dev/null
```
or a the asked client wrote by me:
```
cmake -S ./client -B ./build
cmake --build ./build --target Task2Client --parallel $(nproc)
./build/server/Task2Server
```

if you want to build everything once:
...
./build/server/Task2Server
./build/client/Task2Client
```

## Prerequisites

1. Installed CMake and a C++ compiler
2. Everything else is expected to be downloaded at CMake's configure time.

Problems:
1. Not compilable with MSVC (Microsoft Visual C/C++ compiler) since I use [fmtlog](https://github.com/Arniiiii/fmtlog_cmake_fix) which I haven't updated that properly to be it compilable at Windows with MSVC. Though, you can try `g++` at Windows, it seems to work just fine.
